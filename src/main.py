...
def recognize(
    out_file, most_common, coord_imgs, imgs_with_staff, imgs_spacing, imgs_rows
):
    black_names = [
        "4",
        "8",
    ...
    for i, img in enumerate(coord_imgs):
        ...
        for j, prim in enumerate(primitives):
            prim = binary_opening(prim, square(np.abs(most_common - imgs_spacing[i])))
            ...
            if label in black_names:
                ...
                if len(bounds) > 1 and label not in [
                    ...
                    for k in range(len(bounds)):
                        ...
                        if (
                            k + 1 < len(bounds)
                            and (bounds[k][2] - bounds[k + 1][2])
                            > 1.5 * imgs_spacing[i]
                        ):
                            idx, p = estim(
                                boundary[j][0] + bounds[k][2] - imgs_spacing[i] / 2,
                                i,
                                imgs_spacing,
                                imgs_rows,
                            )
